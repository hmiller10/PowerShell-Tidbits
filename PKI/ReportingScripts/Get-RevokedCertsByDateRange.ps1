...
.SYNOPSIS
    Collect list of revoked certificates from CA database within the 
    defined time period.
...
.DESCRIPTION
    ...
    on the number of revoked certificates within the CA database. 
...
.PARAMETER EndDate
    ...
    revoked certs before

.PARAMETER Report
	Switch parameter that tells script to output the results to CSV file
	
.PARAMETER ReportPath
	Path to where script will export CSV file of all revoked certificates
	found during search of date range defined as input parameters

.OUTPUTS
    Console output for number of revoked certificates found or CSV report file dependent on switch selection.

.EXAMPLE 
    PS> Get-RevokedCertsByDateRange.ps1 -CA ca.domain.com -StartDate "01/01/2019 00:00:00" -EndDate "12/31/2019 23:59:59"

.EXAMPLE 
    PS> Get-RevokedCertsByDateRange.ps1 -CA ca.domain.com -StartDate "01/01/2019 00:00:00" -EndDate "12/31/2019 23:59:59" -Report -ReportPath <FullPathToFile>
...
Try
{
	Do
	{
		...
		Get-RevokedRequest -CertificationAuthority $CA -Filter "RequestID -gt $LastID", "Request.RevokedWhen -ge $StartDate", "Request.RevokedWhen -le $EndDate" -Page $pageNumber -PageSize $pageSize -ErrorAction Continue | `
	...
	If (($revokedCerts.Count -gt 0) -and ($PSBoundParameters.ContainsKey('Report')))
	{
		Write-Output ("Total number of revoked certificates on {0} is: {1}" -f $CA, $revokedCerts.Count)
		$revokedCerts | Select-Object -Property RequestID, Request.RequesterName, CommonName, ConfigString, NotAfter, NotBefore, SerialNumber, `
	...
	ElseIf (($revokedCerts.Count -gt 0))
	{
		Write-Output ("Total number of revoked certificates on {0} is: {1}" -f $CA, $revokedCerts.Count)
	}
	Else
	{
		Write-Output ("There are no revoked certificates {0} from {1} until {2}." -f $CA, $StartDate, $EndDate)
	}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC4+dgF8LyuyehL
# EomECYkdWEUmM7TA6chJulOCuCwSeKCCLjkwggNfMIICR6ADAgECAgsEAAAAAAEh
...
# MC8GCSqGSIb3DQEJBDEiBCADpAIZWNyS8HSKzk2z2eBfMKmmxTCJJaHHKubhX2Sd
# vTANBgkqhkiG9w0BAQEFAASCAQCRlj3stWMdYLzuJaZMZiK9kEawbXWM5a2U6Efz
# nyIxoKyJ2uWpO1eygiKdrwcxw6zTk+ZfJ+Xfn+XtSkp1KE1VBRRoob7HSsIg37rZ
# S/KGk7Y7dkLuwuVwSulY1BZ5yaZHQYIdyE0ORvYu9Z2guA3PhoQlk9fbbyl0a0Ec
# 2G1O2OZTPCJ3BkmBx0t/0Vsg6KgNPhA3jZffCnsaWEDBeazJlO3CS5bDBOdQOLXv
# T3U8HBQULmooiwt4NtSBUI1q+gTKw7BYMW/n51a2l/VoGzkIiNuopzLPo6vjr9+K
# ZBZnK2SLA9s4SSLmFjP+OAHgQiWSSKuvaKhpo5jLHJLTreGfoYIDcDCCA2wGCSqG
...
# DTIxMTAxODIwNTk0NlowLQYJKoZIhvcNAQk0MSAwHjANBglghkgBZQMEAgEFAKEN
# BgkqhkiG9w0BAQsFADAvBgkqhkiG9w0BCQQxIgQgE58BNwbridZbClHyf4eVOKp/
# kTu4ciHLhENVnWHvDYMwgaQGCyqGSIb3DQEJEAIMMYGUMIGRMIGOMIGLBBTdV7Wz
...
# AQsFAASCAYDcrvQoOe932uLRiAoze6KxMeJVacgRSYXo/Kru5cJApVYRUBJOdt4O
# hyjewN0SDle8SAM5kccwUbIXDc5cwBvBDqtclmQnX4Ej0zztwOFWUfTnhrWqtp1p
# S0yvSPS+url+Np9JvbLrJHaEvQHb0jFYL7YVLBZAhi9C6X9p8iWvN5kIrHcy/q0+
# Fk3ON7eB4WAY6I11W6bT9UJhdJt3br/ghaucTfWvFDSh+1n9keci/BrQWfj9PBBu
# 9VFRhgaluHjRDN6jrBjCQ8riJI6SxetnptL3hKs+gBMDLtRsTxErEEIZIq6OqSnc
# 7w2g5fpNuTdNzXWAHGb9DVxcZidn99toMVxflhF/ZpVc6Yj4J8dKFq5Qi8uuKSJW
# CcbFky6kNASBnHrNYL7p25Tqpkvyc0P7tWLH1eJbb7jWq2T99lSW2lyOsI2g4blB
# IwKY3vSk3MU8Zritcx9IEMFWP6U6/CKlxb8wIRby7rBOLNQ6bTe0elVmzfUPKO+u
# 5rJmeUqj3ws=
