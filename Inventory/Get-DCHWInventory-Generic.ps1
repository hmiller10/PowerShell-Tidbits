<#
	...
	.EXAMPLE
	PS C:\> Get-DCHWInventory-Generic.ps1 -Passthru -Credential $creds
...
foreach ($domain in $objComputerDomain.Forest.Domains)
{
	...
	$Status = "Processing domain {0} of {1}: {2}" -f $count, $objComputerDomain.Forest.Domains.Count, $domain.Name.ToString()
	$PercentComplete = ($count / $objComputerDomain.Forest.Domains.Count * 100)
	Write-Progress -Activity $ActivityMessage -Status $Status -PercentComplete $PercentComplete -Id 1
	...
	foreach ($adObject in $colADObjects)
	{
		...
		$ActivityMessage = "Getting domain controller, please wait..."
		$StatusMessage = "Processing {0} of {1}: {2}" -f $count2, $colADObjects.count, $serverDirectoryEntry.dnsHostName.ToString()
		...
		if ($PsBoundParameters.containskey('Credential')) { $cimParams.add('Credential', $Credential) }
		...
		if ($objProperties -ne $null) { $colResults += $objProperties }
		if ($PassThru -eq $true -and $objProperties -ne $null) { $objProperties | Select-Object DnsHostName, IPAddress, ADSite }
		$count2++
	}
	$count++
}
