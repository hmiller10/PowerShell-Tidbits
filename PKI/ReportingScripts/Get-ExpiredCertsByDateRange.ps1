...
.EXAMPLE 
    PS> Get-ExpiredCertsByDateRange.ps1 -CA ca.domain.com -StartDate MM/dd/yyyy HH:mm:ss -EndDate MM/dd/yyyy HH:mm:ss

.EXAMPLE 
    PS> Get-ExpiredCertsByDateRange.ps1 -CA ca.domain.com -StartDate MM/dd/yyyy HH:mm:ss -EndDate MM/dd/yyyy HH:mm:ss -Report -ReportPath <FullPathToFile>
...
try
{
	...
	if (($expiredCerts.Count -gt 0) -and ($PSBoundParameters.ContainsKey('Report')))
	{
		Write-Output ("Total number of expired certificates on {0} is: {1}" -f $CA, $expiredCerts.Count)
		$expiredCerts | Select-Object -Property RequestID, Request.RequesterName, CommonName, ConfigString, NotAfter, NotBefore, SerialNumber, `
	...
	{
		Write-Output ("There are no expired certificates {0} from {1} until {2}." -f $CA, $StartDate, $EndDate)
	}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAd+A+iOE/Ee1bu
# lA/5kdFfySra+P3ymGg9wbX4bH2s66CCLjkwggNfMIICR6ADAgECAgsEAAAAAAEh
...
# MC8GCSqGSIb3DQEJBDEiBCB3iFYeNZByAfE7MZMVW7ezd3BNSuNM9QnjasB8tNyB
# 5zANBgkqhkiG9w0BAQEFAASCAQAbzOLsWDQE9K26dRWqWld9uuB7923trr+JDHW/
# xzwU8OdZwxcLajrxaAxbTKcexM+RjUec8FBvLJWbY6inA8zlpYOOZebn6bEdzCBH
# E73tKF++Xp1HSZtj47bExNLj8zV1cmVQHLNgBFMMdZM06hXbcUVCf1PaflOIhvpe
# T6JEGi/fseuNTQlAEe5GMj7DkuWwQqUriV1WosofY3gdEwgnq+q1tqfOxHeKHkMv
# PXhsXHz7DmOc5/03VqFZ/1MrtsEKXG6IITd9nW3MyVXJp2o//bFc3xv/1svZVxXB
# c3+OAuo1KKitIAmLGe76RalgZ6O+y9dK1MDRsJW0jVSMyrsooYIDcDCCA2wGCSqG
...
# DTIxMTAxODIxMDExOVowLQYJKoZIhvcNAQk0MSAwHjANBglghkgBZQMEAgEFAKEN
# BgkqhkiG9w0BAQsFADAvBgkqhkiG9w0BCQQxIgQgO1iP73FsUc5UvcjGTIzk/wqy
# 9Xmbx3EEUh6jcQpdCa4wgaQGCyqGSIb3DQEJEAIMMYGUMIGRMIGOMIGLBBTdV7Wz
...
# AQsFAASCAYB7wMrkaiVEmRCqQa8SojzmhNHubOva08BnNdqvBUdOshd7d3c53YN0
# GZcLgdYBZh0FjO8rKwMe2GMMUUbME5XA+B597hALacicu7NV2pvCYX3YjhjeCrZw
# HE9zpNoFWu3XBSZCT5VA2pRsSVgOj0HXYQTjRC0PJKD1e6H06KEq96HtVTKNIRrt
# V4aN9+UgiNAoClxdSqUbAdoTrO3HCD+FFGJnTMIOefyUSS1fxOje91XfIIGT4UEh
# YuNzZKtPvfVMtMjrlZjH2JqK1q30v0waJQpiKw2pA+/Fo/XxcfcNmok2NlZAtZ4J
# scCbARY22QMYHpiEEyCQZ25Gbbcn7jLXbZgN4HxzoyPzecVltBo56UyqSMPR98Mv
# B5RkBNS+SBD0X9nZwfFRqspsiG0oC4qhcVCO4M5FNPGYLgEO5d8AN3jevOvMlb+n
# J79y0pDiXOICV6dYzb5ud6OgX5gU9UW7Ued1AyqeJsexY3J440UIrJXoGuMQrUmH
# a1U5ri0EmI8=
