#Requires -Module DirectoryServices, HelperFunctions
...
Catch
{
	...
	Catch
	{
		Throw "HelperFunctions PS module could not be loaded. $($_.Exception.Message)"
	}
}
...
try
{
	...
	Import-Module -Name DirectoryServices -Force -ErrorAction Stop
	...
	switch ($PSCmdlet.ParameterSetName)
	{
		"ParamSetComputerName"
		{
			...
			foreach ($CA in $CertificateAuthority)
			{
				...
				if ((Test-Connection $CA -Count 1 -Quiet) -eq $true)
				{
					$params = @{
						ComputerName = $CA
			...
			$OutputFile = "{0}_{1}_{2}.csv" -f $dtmScriptStartTimeUTC.ToString("yyyy-MM-dd_HH.mm.ss"), (([System.IO.Path]::GetFileNameWithoutExtension($myinv.ScriptName.ToString())).ToString().Replace(" ", "").Replace("-", "")), "for_$($ComputerName)"
		}
